    ...
    const result = await db.collection('order-details').aggregate([
        {
            $group: {
                ...
                totalDiscount: { $sum: { $multiply: ["$Quantity", "$Discount", 100] } }
            }
        },
        {
            $project: {
                ...
                "Total Order Discount, %": { $round: [{ $divide: ["$totalDiscount", "$totalPrice"] }, 3] }
            }
        },
        { $sort: { "Order Id": -1 } }
    ]).toArray()
    ...
    const result = await db.collection('orders').aggregate([
        ...
        },
        {
            $project: {
                ...
                "% of all orders": {
                    $round: [{ $divide: [{ $multiply: ["$TotalNumberOfOrders", 100] }, ordersCount] }, 3]
                }
            }
        },
...
async function task_1_13(db) {
    const totalProducts = await db.collection('products').aggregate([
        { $count: "TotalOfCurrentProducts" }
    ]).toArray();

    const discontinuedProducts = await db.collection('products').aggregate([
        { $match: { Discontinued: 1 } },
        { $count: "TotalOfDiscontinuedProducts" }
    ]).toArray();

    const result = totalProducts.concat(discontinuedProducts);
